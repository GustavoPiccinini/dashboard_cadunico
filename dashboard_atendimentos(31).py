...
CADUNICO_COLS = {
    ...
    "familia_id":    "d.cod_familiar_fam",
    "cd_ibge":       "d.cd_ibge",
    "localidade":    "d.nom_localidade_fam",
    "fx_rfpc":       "d.fx_rfpc",
    "renda_media":   "d.vlr_renda_media_fam",
    "renda_total":   "d.vlr_renda_total_fam",
}

# Município alvo do dashboard (Jacarezinho/PR — código IBGE)
MUNICIPIO_ALVO_IBGE = "4111803"
MUNICIPIO_ALVO_NOME = "Jacarezinho"

# Faixas de renda per capita (código d.fx_rfpc) conforme classificação do CadÚnico
FAIXA_RENDA_LABELS = {
    "1": "Pobreza 1 (até R$ 109)",
    "2": "Pobreza 2 (de R$ 109 a R$ 218)",
    "3": "Baixa Renda (de R$ 218 a R$ 810)",
    "4": "Acima da Baixa Renda (mais de R$ 810)",
}
...
_CADUNICO_KEYWORDS = {
    ...
    "familia_id":    ["codfamiliarfam", "codfamiliar"],
    "cd_ibge":       ["cdibge", "codibge"],
    "localidade":    ["nomlocalidadefam", "localidade"],
    "fx_rfpc":       ["fxrfpc"],
    "renda_media":   ["vlrrendamediafam", "rendamedia"],
    "renda_total":   ["vlrrendatotalfam", "rendatotal"],
}
...
def filtro_ref(ref_valor, coluna: str = "ref_cad") -> str:
    """Retorna condição SQL para filtrar por competência, ou '1=1' se 'Todos os meses' estiver selecionado."""
    if ref_valor == TODOS_MESES:
        return "1=1"
    return f"{coluna} = '{esc(ref_valor)}'"
...
# ABAS
# ══════════════════════════════════════════════
aba_reg, aba_alertas, aba_recorr, aba_cad, aba_bf, aba_fora, aba_exp = st.tabs(["📄 Individuos", "🚨 Alertas", "🔄 Recorrência Alternada", "🔗 CadÚnico", "💚 Bolsa Família", "🌍 Fora do Município", "📥 Exportar"])
    ...
    if not cadunico_loaded:
        ...
        if not refs_disp:
            ...
            ref_sel = st.selectbox(
                ...
                [TODOS_MESES] + refs_disp,
                index=len(refs_disp),
                help="Selecione o mês de referência do CadÚnico, ou 'Todos os meses' para consolidar todas as competências carregadas."
            )
                    ...
                    sql_dr = f"""
                        WITH cad_ref_raw AS (
                            SELECT
                                ...
                                ROW_NUMBER() OVER (PARTITION BY cpf ORDER BY ref_cad DESC) AS rn
                            FROM cadunico
                            WHERE {filtro_ref(ref_sel)}
                              AND cpf IS NOT NULL AND cpf NOT IN ('', 'nan', 'None')
                        ),
                        cad_ref AS (
                            SELECT cpf, nome, nis, nascimento FROM cad_ref_raw WHERE rn = 1
                        ),
                        cpfs_atendidos AS (
                        ...
                        ORDER BY c.nome
                    """
                    # Conta o total real (sem limite) antes de truncar a exibição
                    total_dr = con.execute(f"SELECT COUNT(*) FROM ({sql_dr}) t").fetchone()[0]

                    LIMITE_EXIBICAO = 10000
                    df_dr = con.execute(sql_dr + f" LIMIT {LIMITE_EXIBICAO}").df()

                    total_cad_ref = con.execute(
                        f"SELECT COUNT(DISTINCT cpf) FROM cadunico WHERE {filtro_ref(ref_sel)} AND cpf IS NOT NULL"
                    ).fetchone()[0]
                    total_atendidos_no_cad = total_cad_ref - total_dr

                    m1, m2, m3 = st.columns(3)
                    m1.metric("No CadÚnico nesta competência", f"{total_cad_ref:,}")
                    m2.metric("🔴 Nunca atendidos (demanda reprimida)", f"{total_dr:,}")
                    m3.metric("✅ Já atendidos alguma vez", f"{total_atendidos_no_cad:,}")

                    if not df_dr.empty:
                        pct = total_dr / total_cad_ref * 100 if total_cad_ref > 0 else 0
                        st.progress(min(pct / 100, 1.0), text=f"{pct:.1f}% da base do CadÚnico não foi atendida")

                        if total_dr > LIMITE_EXIBICAO:
                            st.caption(
                                f"⚠️ Exibindo os primeiros {LIMITE_EXIBICAO:,} de {total_dr:,} registros "
                                f"(ordenados por nome). Use o botão de exportação para baixar a lista completa."
                            )

                        st.markdown("##### Lista — clique para ver os dados da pessoa")
                        ev_dr = st.dataframe(
                        ...
                        if sel_dr:
                            ...
                            st.caption("🔴 Esta pessoa não possui nenhum atendimento registrado no sistema.")

                        st.markdown("---")
                        # Exporta a lista completa (sem o limite de exibição da tela)
                        df_dr_export = con.execute(sql_dr).df() if total_dr > LIMITE_EXIBICAO else df_dr
                        out_dr = io.BytesIO()
                        with pd.ExcelWriter(out_dr, engine="openpyxl") as writer:
                            df_dr_export.to_excel(writer, index=False, sheet_name="Demanda Reprimida")
                        st.download_button(
            ...
            with sub_nis:
                ...
                if not c_nis:
                    ...
                    try:
                        sql_nis = f"""
                            WITH cad_sem_cpf_raw AS (
                                SELECT nis, nome, rg, nascimento,
                                       ROW_NUMBER() OVER (PARTITION BY nis ORDER BY ref_cad DESC) AS rn
                                FROM cadunico
                                WHERE {filtro_ref(ref_sel)}
                            ...
                            cad_sem_cpf AS (
                                SELECT nis, nome, rg, nascimento FROM cad_sem_cpf_raw WHERE rn = 1
                            ),
                            atend_nis AS (
    ...
    if not cadunico_loaded:
        ...
        if not tem_marc_fam and not tem_marc_pes:
            ...
            if not refs_bf:
                ...
                ref_bf = st.selectbox(
                    "📅 Competência", [TODOS_MESES] + refs_bf, index=len(refs_bf), key="ref_bf"
                )
                ...
                try:
                    total_ref = con.execute(
                        ...
                        f"WHERE {filtro_ref(ref_bf)} AND cpf IS NOT NULL"
                    ).fetchone()[0]

                    total_bf = con.execute(
                        f"SELECT COUNT(DISTINCT cpf) FROM cadunico "
                        f"WHERE {filtro_ref(ref_bf)} AND cpf IS NOT NULL AND ({filtro_bf})"
                    ).fetchone()[0]

                    if tem_marc_fam and tem_marc_pes:
                        total_bf_fam = con.execute(
                            f"SELECT COUNT(DISTINCT cpf) FROM cadunico "
                            f"WHERE {filtro_ref(ref_bf)} AND cpf IS NOT NULL AND marc_pbf_fam = '1'"
                        ).fetchone()[0]
                        total_bf_pes = con.execute(
                            f"SELECT COUNT(DISTINCT cpf) FROM cadunico "
                            f"WHERE {filtro_ref(ref_bf)} AND cpf IS NOT NULL AND marc_pbf_pes = '1'"
                        ).fetchone()[0]
                ...
                st.markdown("---")
                sub_bf1, sub_bf2, sub_bf3 = st.tabs([
                ...
                with sub_bf1:
                    ...
                    try:
                        ...
                        sql_bf_dr = f"""
                            WITH bf_ref_raw AS (
                                SELECT cpf, nome, nis, nascimento,
                                       {'marc_pbf_fam,' if tem_marc_fam else ''}
                                       {'marc_pbf_pes,' if tem_marc_pes else ''}
                                       ROW_NUMBER() OVER (PARTITION BY cpf ORDER BY ref_cad DESC) AS rn
                                FROM cadunico
                                WHERE {filtro_ref(ref_bf)}
                                  AND cpf IS NOT NULL AND cpf NOT IN ('', 'nan', 'None')
                                  AND ({filtro_nivel})
                            ),
                            bf_ref AS (
                                SELECT * EXCLUDE (rn) FROM bf_ref_raw WHERE rn = 1
                            ),
                            cpfs_atendidos AS (
                            ...
                            ORDER BY b.nome
                        """
                        total_bf_dr = con.execute(f"SELECT COUNT(*) FROM ({sql_bf_dr}) t").fetchone()[0]
                        df_bf_dr = con.execute(sql_bf_dr + " LIMIT 10000").df()

                        mbdr1, mbdr2 = st.columns(2)
                        mbdr1.metric("🔴 BF nunca atendidos", f"{total_bf_dr:,}")
                        pct_dr = total_bf_dr / total_bf * 100 if total_bf > 0 else 0
                ...
                # ── Sub-aba 3: Faixas de Renda ──
                with sub_bf3:
                    st.markdown("##### Famílias do CadÚnico por Faixa de Renda per capita")
                    try:
                        cad_cols_atual = con.execute("DESCRIBE cadunico").df()["column_name"].tolist()
                        if "fx_rfpc" not in cad_cols_atual:
                            st.warning("Coluna `d.fx_rfpc` não encontrada nos arquivos carregados.")
                        else:
                            id_familia = "familia_id" if "familia_id" in cad_cols_atual else "cpf"

                            df_faixas = con.execute(f"""
                                WITH fam_latest AS (
                                    SELECT {id_familia} AS familia, fx_rfpc,
                                           ROW_NUMBER() OVER (PARTITION BY {id_familia} ORDER BY ref_cad DESC) AS rn
                                    FROM cadunico
                                    WHERE {filtro_ref(ref_bf)}
                                      AND fx_rfpc IS NOT NULL AND CAST(fx_rfpc AS VARCHAR) NOT IN ('', 'nan', 'None')
                                )
                                SELECT
                                    CAST(fx_rfpc AS VARCHAR) AS fx_rfpc,
                                    COUNT(DISTINCT familia) AS total_familias
                                FROM fam_latest
                                WHERE rn = 1
                                GROUP BY CAST(fx_rfpc AS VARCHAR)
                            """).df()

                            df_faixas["Faixa de Renda"] = df_faixas["fx_rfpc"].map(
                                lambda c: FAIXA_RENDA_LABELS.get(str(c).strip(), f"Código {c}")
                            )
                            df_faixas = df_faixas.rename(columns={"total_familias": "Famílias"})

                            ordem = list(FAIXA_RENDA_LABELS.values())
                            df_faixas["_ordem"] = df_faixas["Faixa de Renda"].apply(
                                lambda x: ordem.index(x) if x in ordem else 99
                            )
                            df_faixas = df_faixas.sort_values("_ordem").drop(columns=["_ordem", "fx_rfpc"])

                            st.dataframe(
                                df_faixas[["Faixa de Renda", "Famílias"]],
                                use_container_width=True, hide_index=True
                            )

                            fig_faixas = px.bar(
                                df_faixas, x="Faixa de Renda", y="Famílias",
                                text="Famílias", color="Faixa de Renda"
                            )
                            fig_faixas.update_traces(textposition="outside")
                            fig_faixas.update_layout(showlegend=False, height=420)
                            st.plotly_chart(fig_faixas, use_container_width=True)

                            out_faixas = io.BytesIO()
                            with pd.ExcelWriter(out_faixas, engine="openpyxl") as writer:
                                df_faixas[["Faixa de Renda", "Famílias"]].to_excel(writer, index=False, sheet_name="Faixas de Renda")
                            st.download_button(
                                "⬇️ Exportar faixas de renda", out_faixas.getvalue(),
                                f"faixas_renda_{str(ref_bf).replace('/', '-')}.xlsx",
                                "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                                use_container_width=True, key="dl_faixas_renda"
                            )

                            # ── Detalhamento: lista de pessoas/famílias da faixa escolhida ──
                            st.markdown("---")
                            st.markdown("##### Ver a lista completa de uma faixa")

                            faixas_disponiveis = df_faixas["Faixa de Renda"].tolist()
                            faixa_escolhida = st.selectbox(
                                "Escolha a faixa de renda", faixas_disponiveis, key="faixa_escolhida"
                            )

                            codigo_faixa = next(
                                (cod for cod, lbl in FAIXA_RENDA_LABELS.items() if lbl == faixa_escolhida),
                                None
                            )

                            tem_localidade_fx = "localidade" in cad_cols_atual
                            tem_renda_media   = "renda_media" in cad_cols_atual

                            cols_fx = ["nome", "cpf", "nis", "nascimento"]
                            if tem_renda_media:
                                cols_fx.append("renda_media")
                            if tem_localidade_fx:
                                cols_fx.append("localidade")
                            cols_fx_cte = cols_fx + [f"{id_familia} AS familia"]
                            cols_fx_out = cols_fx + ["familia"]

                            df_lista_fx = con.execute(f"""
                                WITH pessoa_latest AS (
                                    SELECT {", ".join(cols_fx_cte)}, fx_rfpc,
                                           ROW_NUMBER() OVER (PARTITION BY cpf ORDER BY ref_cad DESC) AS rn
                                    FROM cadunico
                                    WHERE {filtro_ref(ref_bf)}
                                )
                                SELECT {", ".join(cols_fx_out)}
                                FROM pessoa_latest
                                WHERE rn = 1 AND CAST(fx_rfpc AS VARCHAR) = '{esc(codigo_faixa)}'
                                ORDER BY nome
                            """).df() if codigo_faixa else pd.DataFrame()

                            rename_fx = {
                                "nome": "Nome", "cpf": "CPF", "nis": "NIS", "nascimento": "Nascimento",
                                "renda_media": "Renda Média Familiar", "localidade": "Localidade", "familia": "Família"
                            }
                            df_lista_fx = df_lista_fx.rename(columns=rename_fx)

                            st.metric(f"Total nesta faixa ({faixa_escolhida})", f"{len(df_lista_fx):,}")
                            st.dataframe(df_lista_fx, use_container_width=True, hide_index=True)

                            out_fx = io.BytesIO()
                            with pd.ExcelWriter(out_fx, engine="openpyxl") as writer:
                                df_lista_fx.to_excel(writer, index=False, sheet_name="Faixa de Renda")
                            st.download_button(
                                f"⬇️ Exportar lista — {faixa_escolhida}", out_fx.getvalue(),
                                f"faixa_renda_{codigo_faixa}_{str(ref_bf).replace('/', '-')}.xlsx",
                                "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                                use_container_width=True, key="dl_lista_faixa"
                            )
                    except Exception as e:
                        st.error(f"Erro: {e}")

# ─────────────────────────────────────
# ABA — FORA DO MUNICÍPIO
# ─────────────────────────────────────
with aba_fora:
    st.subheader(f"🌍 Cadastrados no CadÚnico fora de {MUNICIPIO_ALVO_NOME}")
    st.caption(
        f"Lista de famílias/pessoas cujo código IBGE do domicílio (`d.cd_ibge`) é diferente de "
        f"{MUNICIPIO_ALVO_NOME} ({MUNICIPIO_ALVO_IBGE})."
    )
    ...
    if not cadunico_loaded:
        ...
        con = get_con()
        cad_cols_fora = con.execute("DESCRIBE cadunico").df()["column_name"].tolist()

        if "cd_ibge" not in cad_cols_fora:
            st.warning("Coluna `d.cd_ibge` não encontrada nos arquivos carregados.")
        else:
            try:
                refs_fora = con.execute(
                    "SELECT DISTINCT ref_cad FROM cadunico WHERE ref_cad IS NOT NULL ORDER BY ref_cad"
                ).df()["ref_cad"].tolist()
            except Exception:
                refs_fora = []

            ref_fora = st.selectbox(
                "📅 Competência", [TODOS_MESES] + refs_fora,
                index=len(refs_fora), key="ref_fora"
            )

            tem_localidade = "localidade" in cad_cols_fora
            cols_sel = ["nome", "cpf", "nis", "nascimento", "cd_ibge"] + (["localidade"] if tem_localidade else [])

            try:
                df_fora = con.execute(f"""
                    WITH pessoa_latest AS (
                        SELECT {", ".join(cols_sel)},
                               ROW_NUMBER() OVER (PARTITION BY cpf ORDER BY ref_cad DESC) AS rn
                        FROM cadunico
                        WHERE {filtro_ref(ref_fora)}
                          AND cd_ibge IS NOT NULL
                          AND CAST(cd_ibge AS VARCHAR) NOT IN ('', 'nan', 'None')
                    )
                    SELECT {", ".join(cols_sel)}
                    FROM pessoa_latest
                    WHERE rn = 1 AND CAST(cd_ibge AS VARCHAR) != '{MUNICIPIO_ALVO_IBGE}'
                    ORDER BY cd_ibge, nome
                    LIMIT 5000
                """).df()

                rename_map = {
                    "nome": "Nome", "cpf": "CPF", "nis": "NIS",
                    "nascimento": "Nascimento", "cd_ibge": "Código IBGE", "localidade": "Localidade"
                }
                df_fora = df_fora.rename(columns=rename_map)

                st.metric("Total de pessoas fora do município", f"{len(df_fora):,}")
                st.dataframe(df_fora, use_container_width=True, hide_index=True)

                out_fora = io.BytesIO()
                with pd.ExcelWriter(out_fora, engine="openpyxl") as writer:
                    df_fora.to_excel(writer, index=False, sheet_name="Fora do Município")
                st.download_button(
                    "⬇️ Exportar lista", out_fora.getvalue(),
                    f"fora_municipio_{str(ref_fora).replace('/', '-')}.xlsx",
                    "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    use_container_width=True, key="dl_fora_municipio"
                )
            except Exception as e:
                st.error(f"Erro: {e}")
